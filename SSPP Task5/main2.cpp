...
int main(int argc, char** argv) // A*B = C    ./main <namefile A> <namefile B> <namefile C> 
{
	...
	int dimcube[3], period[3], reorder;
	dimcube[0] = pow(numprocs,1.0/3);
	dimcube[1] = pow(numprocs,1.0/3);
	dimcube[2] = pow(numprocs,1.0/3);
	reorder = 0;
	period[0] = 0;
	period[1] = 0;
	period[2] = 0;
	...
	remain_dims_linei[0] = 1;
	remain_dims_linei[1] = 0;
	remain_dims_linei[2] = 0;
	MPI_Cart_sub(cube, remain_dims_linei, &linei); // SUB Linei;

	int remain_dims_linej[3];
	remain_dims_linej[0] = 0;
	remain_dims_linej[1] = 1;
	remain_dims_linej[2] = 0;
	MPI_Cart_sub(cube, remain_dims_linej, &linej); // SUB Linej;

	int remain_dims_stlb[3];
	remain_dims_stlb[0] = 0;
	remain_dims_stlb[1] = 0;
	remain_dims_stlb[2] = 1;
	...
	int n = pow(numprocs, 1.0/3); // count i j k

	int block = nA/n;
	int osta = nA%n;


	//cout << myid << "=" << getk(myid,numprocs) << endl;
	int cordinate[3];
				...
				if( cordinate[1] == n-1 )
				{
				...
				} else
					{
						...
						for(int i = 0; i < block; i++)
						{
							...
							MPI_File_seek(B, (mB - cordinate[1]*block)*sizeof(double) , MPI_SEEK_CUR);
						}
					}	
